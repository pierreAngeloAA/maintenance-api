...
RSpec.describe "Api::V1::Client::Risks", type: :request do
  ...
  it "devuelve el riesgo de cada pieza del vehiculo" do
    get "/api/v1/client/vehicles/#{vehicle.id}/risks", headers: headers
  ...
  it "acepta un tramo distinto por parametro" do
    get "/api/v1/client/vehicles/#{vehicle.id}/risks", params: { horizon: 5_000 }, headers: headers

    expect(json["risks"].first["horizon"]).to eq(5000)
  end

  it "ignora un tramo invalido en vez de reventar" do
    get "/api/v1/client/vehicles/#{vehicle.id}/risks", params: { horizon: "-3" }, headers: headers
  ...
  it "devuelve lista vacia si el vehiculo no tiene piezas parametrizadas" do
    ...
    get "/api/v1/client/vehicles/#{sin_perfil.id}/risks", headers: headers

    expect(json["risks"]).to eq([])
  end

  it "responde 404 si el vehiculo no existe" do
    get "/api/v1/client/vehicles/0/risks", headers: headers
  ...
  describe "aislamiento entre usuarios" do
    it "responde 404 sobre el vehiculo de otro usuario" do
      ...
      get "/api/v1/client/vehicles/#{ajeno.id}/risks", headers: headers

      expect(response).to have_http_status(:not_found)
    end

    it "responde 401 sin token" do
      get "/api/v1/client/vehicles/#{vehicle.id}/risks"
